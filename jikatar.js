...
const viewToggle = document.querySelector('[data-view-toggle="jikatar"]')
...
const buildCard = (post, index) => {
  ...
  return `
    <article class="post-card" data-post-id="${post.id}" data-index="${index}" data-reveal style="--delay: ${index * 0.05}s;">
      <img class="post-thumb" src="${getPreviewImage(index)}" alt="${post.title || 'Essay'}" loading="lazy" decoding="async">
  ...
  if (!postsCache || postsCache.length === 0) {
    container.innerHTML = `
      <article class="empty-state" data-reveal>
        <h3 class="section-title">まだ投稿がありません</h3>
        <p class="section-lead">新しい投稿が公開されるまでお待ちください。</p>
      </article>
    `
    return
  }
...
const openModal = (post, index) => {
  ...
  modalImage.alt = post.title || 'Essay'
  if (modalMeta) {
  ...
  if (modalActions) {
    const tags = parseTags(post.tags)
    modalActions.innerHTML = tags.map((tag) => `<span class="tag">${tag}</span>`).join('')
  }
